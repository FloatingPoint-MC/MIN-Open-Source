...
public class GermPartyRequest implements GermComponent {
    private final String playerName;
    private final GermModIconButton accept, deny;

    public GermPartyRequest(String playerName) {
        this.playerName = playerName;
        accept = new GermModIconButton("btok", "min/hyt/page/accept.png", 12, 12) {
            @Override
            protected void whenClick() {
                Minecraft.getMinecraft().player.connection.sendPacket(new CPacketCustomPayload("germmod-netease",
                        new PacketBuffer(Unpooled.buffer().writeInt(26))
                                .writeString("GUI$team_request_list@bt_accept")
                                .writeString("{\"player_name\":\"" + playerName + "\"}")
                ));
            }
        };
        deny = new GermModIconButton("btno", "min/hyt/page/deny.png", 12, 12) {
            @Override
            protected void whenClick() {
                Minecraft.getMinecraft().player.connection.sendPacket(new CPacketCustomPayload("germmod-netease",
                        new PacketBuffer(Unpooled.buffer().writeInt(26))
                                .writeString("GUI$team_request_list@bt_deny")
                                .writeString("{\"player_name\":\"" + playerName + "\"}")
                ));
            }
        };
    ...
    public void drawComponent(String parentUuid, int x, int y, int mouseX, int mouseY) {
        ...
        accept.drawComponent(parentUuid, x + 40, y, mouseX, mouseY);
        deny.drawComponent(parentUuid, x + 68, y, mouseX, mouseY);
    }

    @Override
    public void mouseClicked(String parentUuid) {
        accept.mouseClicked(parentUuid);
        deny.mouseClicked(parentUuid);
    }
