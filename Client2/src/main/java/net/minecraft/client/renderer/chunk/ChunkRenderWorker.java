...
public class ChunkRenderWorker implements Runnable {
    ...
    protected void processTask(final ChunkCompileTaskGenerator generator) throws InterruptedException {
        ...
        try {
            ...
            if (blockpos1.add(8, 8, 8).distanceSq(blockpos) > 576.0D) {
                ...
                if (this.isChunkNotExisting(blockpos$mutableblockpos.setPos(blockpos1).move(EnumFacing.WEST, 16), world) || this.isChunkNotExisting(blockpos$mutableblockpos.setPos(blockpos1).move(EnumFacing.NORTH, 16), world) || this.isChunkNotExisting(blockpos$mutableblockpos.setPos(blockpos1).move(EnumFacing.EAST, 16), world) || this.isChunkNotExisting(blockpos$mutableblockpos.setPos(blockpos1).move(EnumFacing.SOUTH, 16), world)) {
                    return;
                }
            }
    ...
    private boolean isChunkNotExisting(BlockPos pos, World worldIn) {
